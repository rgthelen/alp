            ...
            if fn_id in fns:
                try:
                    ...
                    if isinstance(declared_inputs, str):
                        ...
                        # The VM binds this to a variable called "in" in the environment
                        # Pass the input_values dict directly as the inbound value
                        ...
                        if len(declared_inputs) == 0:
                            ...
                            # Single named input (e.g., "in": {"expr": "str"})
                            # Check if we should unwrap single values
                            if len(input_values) == 1:
                                # If UI sends single value matching the single input name
                                input_name = next(iter(declared_inputs.keys()))
                                if input_name in input_values:
                                    inbound_data = input_values
                                else:
                                    # UI field name doesn't match function input name
                                    # Wrap the value with the expected name
                        ...
                        # Default: pass input_values as-is
                        inbound_data = input_values
